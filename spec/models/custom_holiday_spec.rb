...
RSpec.describe 'Model CustomHoliday', type: :model do
  describe 'method include?' do
    let!(:custom_holiday_1) { FactoryBot.create(:custom_holiday, date: Date.today - 1) }
    let!(:custom_holiday_2) { FactoryBot.create(:custom_holiday, date: Date.today - 2) }

    context '引数の日付がcustom_holidayに登録されているとき' do
      it 'trueを返す' do
        expect(CustomHoliday.include?(custom_holiday_1.date)).to eq true
      end
    end
